...
public class CostCardinalityMaxDC extends AbstractCPConstraint {
    ...
    public void propagate() {
        ...
        MinCostMaxFlow minCostMaxFlow = new MinCostMaxFlow(costNetworkFlow, capMaxNetworkFlow, H, nVars);

        minCostMaxFlow.run(0, nVars + nValues + 1);

        minCostAssignment=minCostMaxFlow.getTotalCost();

        if (minCostAssignment > H) throw new InconsistencyException();
        ...
        //TODO: build residual graph

        System.out.println("totalCost: " + minCostMaxFlow.getTotalCost());
        System.out.println("totalFlow: " + minCostMaxFlow.getTotalFlow());
    ...
    private void buildNetworkFlow(int[][] costNetworkFlow, int[][] capMaxNetworkFlow) {
        ...
        for (int i = 0; i < nVars; i++) { // from variables to values
            numValues[i] = x[i].fillArray(domains[i]);
            for (int j = 0; j < numValues[i]; j++) {
                capMaxNetworkFlow[i + 1][nVars + 1 + j] = 1; // capacity of 1 for each variable to value edge
                costNetworkFlow[i + 1][nVars + 1 + j] = costs[i][domains[i][j]]; // cost of assigning variable i to value j
            }
        }

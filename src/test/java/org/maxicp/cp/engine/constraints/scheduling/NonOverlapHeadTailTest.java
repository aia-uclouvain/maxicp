...
class NonOverlapHeadTailTest extends CPSolverTest {
    ...
    public void testOK(CPSolver cp) {
        ...
        cp.post(new NoOverlapHeadTail(vars));
    ...
    public void testTimeoutBasicFiltering(CPSolver cp) {
        ...
        assertDoesNotThrow(() -> cp.post(new NoOverlapHeadTail(intervals)));
    }
    ...
    @Test
    public void testSameSolutionsAsDecompositionOptionalsDebug() {
        System.out.println("here");
        int[] duration = new int[] {1,2,3};
        // 3*2*1 = 6*8 = 48 solutions expected
    ...
    public void testDebug() {
        ...
        for (int i = 0 ; i < 3 ; i++) {
            intervals[i] = makeIntervalVar(cp, i+1);
            intervals[i].setStartMax(6);
            intervals[i].setPresent();
        }
        ...
        cp.post(neq(start(intervals[0]),0)); // debug
        cp.post(eq(start(intervals[0]),1)); // debug

        cp.post(neq(start(intervals[1]),2)); // debug
        cp.post(neq(start(intervals[1]),3)); // debug
        /*
        cp.post(eq(start(intervals[1]),4)); // debug
        cp.post(eq(start(intervals[2]),6)); // debug
        */
    }

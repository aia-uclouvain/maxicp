...
launch_solver=" java -cp target/maxicp-0.0.1-jar-with-dependencies.jar org.maxicp.cp.examples.raw.distance.OTSPBench"
currentDate=$(date +%Y-%m-%d_%H-%M-%S);
gitShortHash=$(git rev-parse --short HEAD)
outFileOpt="results/otsp/otsp-${currentDate}-${gitShortHash}"
...
mkdir -p "results/otsp"  # where the results will be written
...
do
  ...
  do
    ...
    find data/OTSP/ -type f | sed "s/$/,$distance/"  >> $inputFile
  done
done

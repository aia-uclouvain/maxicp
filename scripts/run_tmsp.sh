...
launch_solver=" java -cp target/maxicp-0.0.1-jar-with-dependencies.jar org.maxicp.cp.examples.raw.distance.TMSPBench"
currentDate=$(date +%Y-%m-%d_%H-%M-%S);
gitShortHash=$(git rev-parse --short HEAD)
outFileOpt="results/tmsp/tmsp-${currentDate}-${gitShortHash}"
# declare -a distanceType=("ORIGINAL" "MIN_INPUT_AND_OUTPUT_SUM" "MIN_DETOUR" "MATCHING_SUCCESSOR")  # -m, each type of distance constraint to try
declare -a distanceType=("MIN_INPUT_AND_OUTPUT_SUM")
mkdir -p "results/tmsp"  # where the results will be written
...
echo "class | instance | variant | best_obj | best_sol | timeout | runtime | n_nodes | n_failures | n_sols | is_completed | solution_list | args " >> $outFileOpt
echo "writing inputs"
...
do
  ...
  do
    ...
    find data/TMSP/ -type f | sed "s/$/,$distance/"  >> $inputFile
  done
done
